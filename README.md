...
We use pre-trained word vectors from a BERT-base (cased version) model, considering that capitalization might also be influential in the English setting. Specifically, the input sentence S will be tokenized into a sub-word token sequence ss = {w1, w2, ..., wn} of length n and encoded with BERT to get the word embedding of wi by averaging the embeddings of its tokens. We add a new POS embedding to utilize the POS information.  We concatenate the two features and pass through BERT to get contextualized representation.


We have two classifiers: a sentiment classifier and an additional aspect classifier. 
...
Hitkul Jangid, Shivangi Singhal, Rajiv Ratn Shah, and Roger Zimmermann. 2018. Aspect-Based Financial Sentiment Analysis using Deep Learning. In WWW ’18 Companion: The 2018 Web Conference Companion, April 23–27, 2018, Lyon, France. ACM, New York, NY, USA, 6 pages. https://doi.org/10.1145/3184558.3191827
